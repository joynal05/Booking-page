...
$formatted_total_price = '<span class="total_price_sum">
' .
    // number_format_i18n( $total_price, $price_decimal_places ) 
    $total_price
...
$current_date = date(TRANSFERS_PHP_DATE_FORMAT, strtotime($current_date) + ($timezone_offset * 60 * 60));
...
<?php if (!empty($availability_entry_dep)) { ?>
    <header class="output step1 step2 color <?php echo (isset($return_date)) ? 'twoway' : ''; ?>">
        <div class="wrap">
            <div>
                <p><?php echo date_i18n($date_format, strtotime($departure_date)); ?> <small><?php esc_html_e('at', 'transfers'); ?></small> <?php echo esc_html($slot_minutes_dep); ?></p>
            ...
            <?php if ($availability_entry_ret != null) { ?>
                <div>
                    <p><?php echo date_i18n($date_format, strtotime($return_date)); ?> <small><?php esc_html_e('at', 'transfers'); ?></small> <?php echo esc_html($slot_minutes_ret); ?></p>
                    ...
                    <div class="row">
                        <?php if ($transfers_plugin_globals->enable_extra_items()) { ?>
                            ...
                            <div class="three-fourth step1">
                                ...
                                if ($departure_extra_items && is_array($departure_extra_items) && count($departure_extra_items) > 0) { ?>
                                    <script>
                                    ...
                                    <table class="data responsive">
                                        ...
                                        foreach ($departure_extra_items as $extra_item) {
                                            ...
                                            $item_price_pre = $departure_is_private ? $transfers_extra_item->get_custom_field('price_private') : $transfers_extra_item->get_custom_field('price_shared');



                                            // custom code start

                                            $tf_str_time1 = "22:00"; // 10.00pm
                                            $tf_str_time2 = "06:00"; // 6.00am
                                            $tf_str_time3 = $slot_minutes_dep; // selected site

                                            $tf_str_time1 = preg_replace("/^([\d]{1,2})\:([\d]{2})$/", "00:$1:$2", $tf_str_time1);
                                            $tf_str_time2 = preg_replace("/^([\d]{1,2})\:([\d]{2})$/", "00:$1:$2", $tf_str_time2);
                                            $tf_str_time3 = preg_replace("/^([\d]{1,2})\:([\d]{2})$/", "00:$1:$2", $tf_str_time3);

                                            sscanf($tf_str_time1, "%d:%d:%d", $tf_hours1, $tf_minutes1, $tf_seconds1);
                                            sscanf($tf_str_time2, "%d:%d:%d", $tf_hours2, $tf_minutes2, $tf_seconds2);
                                            sscanf($tf_str_time3, "%d:%d:%d", $tf_hours3, $tf_minutes3, $tf_seconds3);

                                            $tf_time_range_1 = $tf_hours1 * 3600 + $tf_minutes1 * 60 + $tf_seconds1;
                                            $tf_time_range_2 = $tf_hours2 * 3600 + $tf_minutes2 * 60 + $tf_seconds2;
                                            $tf_time_range_3 = $tf_hours3 * 3600 + $tf_minutes3 * 60 + $tf_seconds3;


                                            if ($tf_time_range_3 >= $tf_time_range_1 || $tf_time_range_3 <= $tf_time_range_2) {

                                                $item_price = $item_price_pre + $item_price_pre * 20 / 100;
                                            } else {

                                                $item_price = $item_price_pre;
                                            }
                                ...
                                <div class="summary">
                                    <div>
                                        ...
                                        <dl>
                                            ...
                                            <dd class="departure_date"><?php echo esc_html(date_i18n($date_format, strtotime($departure_date))); ?> <?php echo esc_html($slot_minutes_dep); ?></dd>
                                    ...
                                    <?php if ($availability_entry_ret != null) { ?>
                                        <div>
                                            ...
                                            <dl>
                                                <dt><?php esc_html_e('Date', 'transfers') ?></dt>
                                                <dd class="return_date"><?php echo esc_html(date_i18n($date_format, strtotime($return_date))); ?> <?php echo esc_html($slot_minutes_ret); ?></dd>
                                                <dt><?php esc_html_e('From', 'transfers') ?></dt>
